...
<body class="bg-zinc-950 text-zinc-100 min-h-screen flex flex-col justify-between">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="flex items-center">
                    <a href="{{ route('guru.dashboard') }}" class="text-2xl font-bold tracking-tight bg-gradient-to-r from-indigo-400 to-violet-400 bg-clip-text text-transparent">
                ...
                <nav class="hidden md:flex items-center gap-6">
                    <a href="{{ route('guru.dashboard') }}" 
                        class="text-sm font-medium transition duration-200 {{ request()->routeIs('guru.dashboard') ? 'text-white border-b-2 border-indigo-500 pb-1' : 'text-zinc-400 hover:text-white' }}">
                        Dashboard
                    </a>
                    <a href="{{ route('guru.history') }}" 
                        class="text-sm font-medium transition duration-200 {{ request()->routeIs('guru.history') ? 'text-white border-b-2 border-indigo-500 pb-1' : 'text-zinc-400 hover:text-white' }}">
                        Riwayat Absen
                    </a>
                    <a href="{{ route('guru.leave') }}" 
                        class="text-sm font-medium transition duration-200 {{ request()->routeIs('guru.leave') ? 'text-white border-b-2 border-indigo-500 pb-1' : 'text-zinc-400 hover:text-white' }}">
                        Ajukan Izin
                    </a>
                </nav>
    ...
    <div class="md:hidden bg-zinc-900 border-t border-zinc-800 sticky bottom-0 z-50 px-4 py-2 flex justify-around items-center">
        <a href="{{ route('guru.dashboard') }}" 
            class="flex flex-col items-center justify-center py-1 transition duration-200 {{ request()->routeIs('guru.dashboard') ? 'text-indigo-400' : 'text-zinc-500 hover:text-zinc-300' }}">
        ...
        <a href="{{ route('guru.history') }}" 
            class="flex flex-col items-center justify-center py-1 transition duration-200 {{ request()->routeIs('guru.history') ? 'text-indigo-400' : 'text-zinc-500 hover:text-zinc-300' }}">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
        ...
        <a href="{{ route('guru.leave') }}" 
            class="flex flex-col items-center justify-center py-1 transition duration-200 {{ request()->routeIs('guru.leave') ? 'text-indigo-400' : 'text-zinc-500 hover:text-zinc-300' }}">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
