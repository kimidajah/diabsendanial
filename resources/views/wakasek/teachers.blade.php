...
<div class="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-sm">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h3 class="text-lg font-bold text-zinc-100">Kelola Akun Guru</h3>
        ...
        <button onclick="toggleModal('add-modal', true)" 
            class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium text-xs py-2.5 px-4 rounded-xl shadow-sm flex items-center gap-1.5 self-start sm:self-center">
            ...
            <tbody class="divide-y divide-zinc-800">
                @forelse ($teachers as $teacher)
                    <tr class="hover:bg-zinc-800">
                        <td class="py-4 px-4 font-semibold text-zinc-250">
                            {{ $teacher->name }}
                        </td>
                        ...
                        <td class="py-4 px-4 text-right space-x-2">
                            <button 
                                ...
                                class="bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-xs px-3 py-1.5 rounded-lg border border-zinc-700 inline-flex items-center gap-1">
                                Edit
                            </button>
                            ...
                            <form action="{{ route('wakasek.teachers.destroy', $teacher->id) }}" method="POST" class="inline">
                                ...
                                <button type="submit" onclick="return confirm('Apakah Anda yakin ingin menghapus akun guru ini? Tindakan ini tidak dapat dibatalkan.')"
                                    class="bg-red-50 border border-red-200 text-red-700 text-xs px-3 py-1.5 rounded-lg inline-flex items-center gap-1">
                                    Hapus
                                </button>
                            </form>
                        </td>
...
<!-- ================= MODAL TAMBAH GURU ================= -->
<div id="add-modal" class="fixed inset-0 z-50 overflow-y-auto hidden bg-zinc-950/80 flex items-center justify-center p-4">
    <div class="bg-zinc-900 border border-zinc-800 w-full max-w-md rounded-2xl p-6 shadow-sm">
        <div class="flex items-center justify-between mb-6 pb-3 border-b border-zinc-800">
            <h4 class="text-base font-bold text-zinc-100">Tambah Akun Guru Baru</h4>
        ...
        <form action="{{ route('wakasek.teachers.store') }}" method="POST" class="space-y-4 text-left">
            ...
            <div>
                ...
                <input type="text" name="name" id="add-name" required placeholder="Ahmad Zulkarnaen, M.Pd"
                    class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
            </div>
            ...
            <div class="flex gap-3 pt-4">
                <button type="button" onclick="toggleModal('add-modal', false)"
                    class="flex-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 py-2.5 rounded-xl text-xs border border-zinc-700">
                    Batal
                </button>
                <button type="submit" 
                    class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2.5 rounded-xl text-xs shadow-sm">
                    Simpan
                </button>
            </div>
        </form>
...
<!-- ================= MODAL EDIT GURU ================= -->
<div id="edit-modal" class="fixed inset-0 z-50 overflow-y-auto hidden bg-zinc-950/80 flex items-center justify-center p-4">
    <div class="bg-zinc-900 border border-zinc-800 w-full max-w-md rounded-2xl p-6 shadow-sm">
        <div class="flex items-center justify-between mb-6 pb-3 border-b border-zinc-800">
            <h4 class="text-base font-bold text-zinc-100">Edit Informasi Guru</h4>
