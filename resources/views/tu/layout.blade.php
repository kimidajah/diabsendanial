...
<body class="bg-zinc-950 text-zinc-100 min-h-screen flex flex-col justify-between">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="flex items-center">
                    <a href="{{ route('tu.scanner') }}" class="text-2xl font-bold tracking-tight text-indigo-600">
                ...
                <nav class="hidden md:flex items-center gap-6">
                    <a href="{{ route('tu.scanner') }}" 
                        class="text-sm font-medium {{ request()->routeIs('tu.scanner') ? 'text-indigo-600 border-b-2 border-indigo-600 pb-1' : 'text-zinc-400 hover:text-zinc-200' }}">
                        Scanner QR
                    </a>
                    <a href="{{ route('tu.reports') }}" 
                        class="text-sm font-medium {{ request()->routeIs('tu.reports') ? 'text-indigo-600 border-b-2 border-indigo-600 pb-1' : 'text-zinc-400 hover:text-zinc-200' }}">
                        Rekap Absensi
                    </a>
                </nav>
    ...
    <div class="md:hidden bg-zinc-900 border-t border-zinc-800 sticky bottom-0 z-50 px-4 py-2 flex justify-around items-center">
        <a href="{{ route('tu.scanner') }}" 
            class="flex flex-col items-center justify-center py-1 {{ request()->routeIs('tu.scanner') ? 'text-indigo-600' : 'text-zinc-400 hover:text-zinc-200' }}">
        ...
        <a href="{{ route('tu.reports') }}" 
            class="flex flex-col items-center justify-center py-1 {{ request()->routeIs('tu.reports') ? 'text-indigo-600' : 'text-zinc-400 hover:text-zinc-200' }}">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
